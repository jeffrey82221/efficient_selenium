...
class _FundNavExtractActor:
    ...
    def request(self, input_tuple):
        try:
            if VERBOSE: print(f'[request {input_tuple[0]}] Start');
            if self._first_try:
            ...
            if VERBOSE: print(f'[request {input_tuple[0]}] _path_extractor built');
            if os.path.exists(self._path_extractor.table_path):
                download_mode = 'partial'
            else:
                download_mode = 'full'
            if VERBOSE: print(f'[request {input_tuple[0]}] {download_mode} pipe built');
            end_pipe = self._build_pipe(fund, url, download_mode=download_mode)
            _ = list(end_pipe)
            if VERBOSE: print(f'[request {input_tuple[0]}] {download_mode} download finish');
            if download_mode == 'partial':
                self._update_h5()
                if VERBOSE: print(f'[request {input_tuple[0]}] h5 updated');
    ...
    def _update_h5(self):
        try:
            ...
            original_table = pd.read_hdf(self._path_extractor.table_path, 'nav')
        ...
        shutil.copyfile(tmp_path_extractor.table_path, self._path_extractor.table_path)
